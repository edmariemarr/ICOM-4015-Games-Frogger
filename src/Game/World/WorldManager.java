...
public class WorldManager {
	...
	private ArrayList<BaseArea> StartingAreas; // Empty and grass area
    ...
    public WorldManager(Handler handler) {
        ...
        StartingAreas.add(new GrassArea(handler, 0));
        StartingAreas.add(new EmptyArea(handler, 0));
        ...
        for(int i=0; i<gridHeight+2; i++) {
        	SpawnedAreas.add(startArea((-2+i)*64));
        }
        	
        player.setX((gridWidth/2)*64);
        player.setY((gridHeight-1)*64);
	...
	// Spawn in Grass and Empty Areas
	private BaseArea startArea(int yPosition) {
		Random rand = new Random();
 
    	BaseArea startArea = StartingAreas.get(rand.nextInt(StartingAreas.size()));
    	
    	if(startArea instanceof GrassArea) {
    		startArea = new GrassArea(handler, yPosition);
    	}
    	else {
    		startArea = new EmptyArea(handler, yPosition);
    	}
		return startArea;
	}

	//Game Over by water
	private void WaterGameOver() {
		for (int j =0; j < SpawnedAreas.size(); j++) {
			if (SpawnedAreas.get(j) instanceof WaterArea) {
				if (player.getPlayerCollision().getY() ==SpawnedAreas.get(j).getYPosition() ) {
					for (int k = 0; k<SpawnedHazards.size();k++) {
						if (player.getPlayerCollision().getX()-64 >= SpawnedHazards.get(k).GetCollision().getX() - 64&& player.getPlayerCollision().getX() +64 <= SpawnedHazards.get(k).GetCollision().getX()+64&&player.getPlayerCollision().getY() >= SpawnedHazards.get(k).GetCollision().getY() -64 && player.getPlayerCollision().getX() <= (SpawnedHazards.get(k).GetCollision().getY()+64 )){
	...
	private void SpawnHazard(int yPosition, BaseArea area) {
		...
		if (area instanceof WaterArea) { //if WaterArea
		if (choice <=2) {
			randInt = 64 * rand.nextInt(9);
			int n = rand.nextInt(4);
			int m = rand.nextInt(3);
			for (int i=0;i<n+1;i++) {
			SpawnedHazards.add(new Log(handler, randInt+((i+1)*(128*m)), yPosition));
			}
			LP = false;
		}
		else if (choice >=5 && LP== false ){
			randInt = 64 * rand.nextInt(9);
			int n = rand.nextInt(4);
			int m = rand.nextInt(3);
			m+=1;
			for (int i=0;i<n+1;i++) {
			SpawnedHazards.add(new LillyPad(handler, randInt+(i*(64*m)), yPosition));
			}
			LP = true;
		}
		else {
		...
		else if (area instanceof GrassArea) { //if GrassArea
			if (choice <=2) {
